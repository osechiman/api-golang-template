...
import (
	...
	"vspro/yellow/entities"
	"vspro/yellow/valueobjects"
)
...
func TestQuestionUsecase_Add(t *testing.T) {
	type fields struct {
		Repository QuestionRepositorer
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := qu.AddQuestion(tt.args.q)
			if (err != nil) != tt.wantErr {
				t.Errorf("AddQuestion() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("AddQuestion() got = %v, want %v", got, tt.want)
			}
		})
	}
}
